...
class TicketCalculator(QWidget):
    ...
    def __init__(self, current_user, current_user_id=None, id_token=None):  # Thêm id_token
        super().__init__()
        self.current_user = current_user
        self.current_user_id = current_user_id
        self.id_token = id_token  # Lưu idToken
    ...
    def closeEvent(self, event):
        try:
            if hasattr(self, 'lock'):
                portalocker.unlock(self.lock)
                self.lock.close()
            # Đăng xuất người dùng khỏi Firebase (trừ admin)
            if self.current_user and self.current_user != "admin" and self.current_user_id:
                try:
                    print(f"Đang đăng xuất user_id: {self.current_user_id}")  # Debug
                    db.child("online_users").child(self.current_user_id).remove(self.id_token)  # Sử dụng id_token
...
if __name__ == "__main__":
    ...
    if login_dialog.exec() == QDialog.DialogCode.Accepted:
        window = TicketCalculator(
            ...
            login_dialog.id_token  # Truyền id_token
        )
        window.show()
        sys.exit(app.exec())
    else:
