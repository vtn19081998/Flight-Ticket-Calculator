...
class LoginDialog(QDialog):
    ...
    def process_login(self, username, password):
        try:
            ...
            status = db.child("online_users").child(user['localId']).get(self.id_token).val()
            if status:
                ...
                if reply == QMessageBox.StandardButton.No:
                    self.reject()
                    return
                # Xóa trạng thái online của phiên cũ
                db.child("online_users").child(user['localId']).remove(self.id_token)

            # Đặt trạng thái online cho phiên mới
            db.child("online_users").child(user['localId']).set({"online": True}, self.id_token)
