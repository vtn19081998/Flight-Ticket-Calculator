...
class TicketCalculator(QWidget):
    ...
    def __init__(self):
        ...
        self.child_price_ratio = 0.7  # Mặc định, sẽ được cập nhật từ config

        # Đọc cấu hình từ file JSON
        self.load_config()

        # Xác định đường dẫn đến thư mục Documents\Flight Ticket Calculator
        documents_path = os.path.join(os.path.expanduser("~"), "Documents", "Flight Ticket Calculator")
        os.makedirs(documents_path, exist_ok=True)  # Tạo thư mục nếu chưa tồn tại

        # Đặt đường dẫn file khóa trong Documents\Flight Ticket Calculator
        ...
        try:
            ...
            # Hiển thị thông báo lỗi và ép buộc thoát ứng dụng
            msg_box = QMessageBox()
            ...
            msg_box.exec()
            self.lock.close()
            sys.exit(1)  # Ép buộc thoát ứng dụng với mã lỗi 1
    ...
    def update_note_label(self, detected_airlines):
        ...
        # Kiểm tra xem có dữ liệu chuyến bay hay không
        ...
        if not has_flight_data:
            # Nếu không có dữ liệu, hiển thị ghi chú cơ bản
        ...
        # Xác định hãng bay cho từng chuyến
        airline1 = None
        airline2 = None
        for airline in self.airlines_config:
            if flight_number1.upper().startswith(airline.upper()):
                airline1 = airline
            if self.is_round_trip and flight_number2.upper().startswith(airline.upper()):
                airline2 = airline

        # Nếu không tìm thấy hãng, sử dụng detected_airlines
        ...
        # Kiểm tra suất ăn dựa trên airline1 và airline2
        has_meal = False
        ...
        # Cập nhật base_text dựa trên việc có suất ăn hay không
        if has_meal:
        ...
        if not self.is_round_trip:
            # Nếu không phải khứ hồi, hiển thị một dòng cho chuyến bay 1
            if airline1:
                note_text += f"● Với mỗi vé {airline1}, được mang {luggage1}<br>"
        else:
            # Nếu là khứ hồi, kiểm tra xem có cùng hãng và cùng hành lý không
            if airline1 == airline2 and luggage1 == luggage2:
                # Nếu cùng hãng và cùng hành lý, hiển thị một dòng chung
                if airline1:
                    note_text += f"● Với mỗi vé {airline1}, được mang {luggage1}<br>"
            else:
                # Nếu khác hãng hoặc khác hành lý, hiển thị hai dòng riêng biệt
                if airline1:
                    note_text += f"● Với mỗi vé {airline1}, được mang {luggage1}<br>"
                if airline2:
                    note_text += f"● Với mỗi vé {airline2}, được mang {luggage2}<br>"
