...
from login import LoginDialog  # Nhập LoginDialog từ login.py
import pyrebase  # Thêm import pyrebase
...
auth = firebase.auth()  # Thêm auth để làm mới token
...
class TicketCalculator(QWidget):
    ...
    def __init__(self, current_user, current_user_id=None, id_token=None, refresh_token=None):
        ...
        self.refresh_token = refresh_token  # Thêm refresh_token
    ...
    def refresh_id_token(self):
        """Làm mới idToken sử dụng refreshToken."""
        try:
            ...
            self.refresh_token = refreshed['refreshToken']  # Cập nhật refreshToken mới
...
if __name__ == "__main__":
    ...
    if login_dialog.exec() == QDialog.DialogCode.Accepted:
        window = TicketCalculator(
            ...
            login_dialog.refresh_token  # Truyền refresh_token
        )
        window.show()
        sys.exit(app.exec())
    else:
