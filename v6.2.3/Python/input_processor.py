...
class InputProcessor:
    ...
    VALID_AIRCRAFT_TYPES = {
        ...
        "Airbus": ["A320", "A320neo", "A318", "A319", "A321", "A321neo", "A330", "A330-200", "A330-300", "A330-900", "A350", "A350-900", "A350-900ULR", "A380", "A359" "A32Q", "A32N", "A333", "A359", "A321B"],
    ...
    def __init__(self, airlines_config, cities_file='data/cities.json'):
        ...
        self._flight_pattern = re.compile(
            rf'(?:\s*({codes_pattern})[-|\s]*(\d{{3,4}})\b(?!\s*giờ))',
            re.IGNORECASE
        )
    ...
    def extract_from_clipboard(self, parent, input_price, input_flight1, input_time1,
        ...
        # Trước tiên, xử lý mã chuyến bay từ phần "Ghi chú"
        ...
        # Chỉ tìm các mã chuyến bay khác nếu không tìm thấy trong "Ghi chú"
        if not flight_codes:
        ...
        # Dự phòng: Phát hiện hãng bay nếu không tìm thấy mã chuyến bay
        if not self.detected_airlines:
            for airline in self.airlines_config:
                airline_name_lower = airline.lower()
                airline_code = self.airlines_config[airline].get('code', '').lower()
