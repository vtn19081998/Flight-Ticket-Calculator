    ...
    def setup_online_status_listener(self):
        ...
        try:
            ...
            def stream_handler(message):
                ...
                if message["data"] is None or (isinstance(message["data"], dict) and message["data"].get("device_id") != self.device_id):
        ...
        def check_online_status():
            ...
            for attempt in range(retries):
                try:
                    ...
                    if not status or status.get("device_id") != self.device_id:
                        QTimer.singleShot(0, self.show_session_terminated_message)
                    return
                except Exception as e:
...
if __name__ == "__main__":
    ...
    if login_dialog.exec() == QDialog.DialogCode.Accepted:
        window = TicketCalculator(
            ...
            login_dialog.device_id  # Truyền device_id từ LoginDialog
        )
        window.show()
        sys.exit(app.exec())
    else:
