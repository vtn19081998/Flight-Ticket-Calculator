...
class TicketCalculator(QWidget):
    ...
    def initUI(self):
        ...
        self.input_flight_number1.setStyleSheet("font-family: Tahoma; background-color: #f5f6fa; border: 1px solid #CCCCCC; border-radius: 5px; padding: 3px;")
        ...
        self.export_format_combo.addItems(["Văn bản chi tiết", "Văn bản ngắn gọn", "JSON"])
    ...
    def copy_text(self, export_format="Văn bản chi tiết"):
        ...
        try:
            ...
            flight1_text = re.sub(r'<[^>]+>', '', self.input_flight1.text()).strip() or "Không xác định"
            flight2_text = re.sub(r'<[^>]+>', '', self.input_flight2.text()).strip() if self.is_round_trip else ""
            ...
            if export_format == "Văn bản ngắn gọn":
                clipboard_content = (
                    f"✈️ THÔNG TIN CHUYẾN BAY:\n"
                    f"♦️ Hãng bay: {airline_name}\n"
                    f"♦️ Lộ trình: {flight1_text}\n"
                    f"♦️ Tổng số khách: {total_guests}\n"
                    f"♦️ Tổng chi phí: {self.result_label.text().strip()}\n"
                )
                if self.is_round_trip and flight2_text:
                    clipboard_content += f"♦️ Lộ trình khứ hồi: {flight2_text}\n"
                ...
                data = {
                    ...
                    "flight_1": {
                        "route": flight1_text,
                        "time": re.sub(r'<[^>]+>', '', self.input_time1.text()),
                        "flight_number": self.input_flight_number1.text(),
                        "plane_type": self.input_plane_type1.text()
                    },
                    "flight_2": {
                        "route": flight2_text,
                        "time": re.sub(r'<[^>]+>', '', self.input_time2.text()),
                        "flight_number": self.input_flight_number2.text(),
                        "plane_type": self.input_plane_type2.text()
                    } if self.is_round_trip else None,
                    "passengers": {
                        "adult": adult_count,
                        "child": child_count,
                        "infant": infant_count
                    ...
                    "pricing": {
                        ...
                        "discount_percent": float(voucher_value),
                        "total_cost": float(self.result_label.text().replace("TỔNG CHI PHÍ TOÀN HÀNH TRÌNH: ", "").replace(" VNĐ", "").replace(",", ""))
                    },
                    "notes": note_text_cleaned
                }
                clipboard_content = json.dumps(data, indent=2, ensure_ascii=False)
    ...
    def extract_from_clipboard(self):
        try:
            ...
            logo_paths = {
                ...
                "Vietravel Airlines": "images/viettravel.png",
