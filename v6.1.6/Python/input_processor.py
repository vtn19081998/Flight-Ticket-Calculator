...
class InputProcessor:
    ...
    def _load_cities_and_compile_regex(cls, cities_file):
        ...
        # Cải tiến regex để khớp tên thành phố, mã sân bay, và nhà ga (T1, T2,...)
        cities_pattern = '|'.join(
            f"(?:(?:tp\\s*)?{re.escape(city['city_name'].lower().replace('tp ', ''))}(?:\\s*\\({re.escape(city['code'])}\\)(?:\\s*\\(t[1-4]\\))?|\\s*\\(t[1-4]\\))?|(?:tp\\s*)?{re.escape(city['city_name'].lower().replace('tp ', ''))})"
    ...
    def __init__(self, airlines_config, cities_file='data/cities.json'):
        ...
        self.day_pattern = re.compile(r'(thứ\s*(?:hai|ba|tư|năm|sáu|bảy)|chủ nhật|c\.nhật|t\.\w+)', re.IGNORECASE)
