...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.id_token = None  # Thêm biến lưu idToken
    ...
    def attempt_login(self):
        ...
        # Disable nút và báo loading
        self.login_button.setEnabled(False)
        self.login_button.setText("Đang kiểm tra...")

        # Sử dụng QTimer.singleShot để tránh freeze giao diện
    ...
    def process_login(self, username, password):
        try:
            ...
            if username.lower() == "admin" and password == "Abc13579@":
                ...
                self.id_token = None  # Admin không cần token
            ...
            self.id_token = user['idToken']  # Lưu idToken

            # Kiểm tra trạng thái đăng nhập trên Database
            status = db.child("online_users").child(user['localId']).get(user['idToken'])
            if status.val() == "online":
                QMessageBox.warning(self, "Lỗi", "Tài khoản này đang được sử dụng ở một thiết bị khác.")
                return

            # Cập nhật trạng thái thành "online" với idToken
            db.child("online_users").child(user['localId']).set("online", user['idToken'])
    ...
    def closeEvent(self, event):
        # Nếu người dùng đã đăng nhập và không phải là admin, đăng xuất khỏi Firebase
        if self.current_user and self.current_user != "admin":
            try:
                db.child("online_users").child(self.current_user_id).remove()  # Đăng xuất
            except Exception as e:
