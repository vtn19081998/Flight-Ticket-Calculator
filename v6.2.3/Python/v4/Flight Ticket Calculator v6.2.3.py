...
class TicketCalculator(QWidget):
    ...
    def __init__(self, current_user, current_user_id=None, id_token=None, refresh_token=None):
        super().__init__()
        ...
        # Thêm lắng nghe sự kiện Firebase cho trạng thái online (chỉ cho người dùng không phải admin)
        if self.current_user != "admin" and self.current_user_id and self.id_token:
    ...
    def setup_online_status_listener(self):
        ...
        try:
            def stream_handler(message):
                # Nếu trạng thái online bị xóa, hiển thị thông báo và thoát ứng dụng
            ...
            # Lắng nghe thay đổi trong online_users/<user_id>
            db.child("online_users").child(self.current_user_id).stream(stream_handler, self.id_token)
    ...
    def refresh_id_token(self):
        ...
        for attempt in range(retries):
            try:
                ...
                continue
        QMessageBox.critical(self, "Lỗi", "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.")
        self.close()
        login_dialog = LoginDialog()
        if login_dialog.exec() == QDialog.DialogCode.Accepted:
            self.current_user = login_dialog.current_user
            self.current_user_id = login_dialog.current_user_id
            self.id_token = login_dialog.id_token
            self.refresh_token = login_dialog.refresh_token
            self.initUI()
            self.show()
            return self.id_token
        else:
    ...
    def closeEvent(self, event):
        try:
            if hasattr(self, 'lock'):
                portalocker.unlock(self.lock)
                self.lock.close()
            if self.current_user and self.current_user != "admin" and self.current_user_id:
                try:
                    if not self.id_token:
                        self.id_token = self.refresh_id_token()
                    if self.id_token:
                        print(f"Đang đăng xuất user_id: {self.current_user_id}")
    ...
    def calculate_total(self):
        try:
            ...
            history_entry = {
                ...
                'plane_type1': self.input_plane_type1.text(),
