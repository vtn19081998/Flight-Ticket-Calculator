...
from PyQt6.QtWidgets import (
    ...
    QTableWidgetItem, QHeaderView, QMessageBox, QGroupBox, QGridLayout, QCheckBox, QComboBox, QFileDialog
)
...
class TicketCalculator(QWidget):
    ...
    def check_running_instances(self):
        ...
        for proc in psutil.process_iter(['pid', 'exe', 'name', 'cmdline']):
            try:
                ...
                # Kiểm tra xem tiến trình có phải là Flight Ticket Calculator
                if (proc_exe == current_exe or
                    ...
                    running.append(proc)
                    # Thu thập thông tin cho QMessageBox
                    create_time = datetime.fromtimestamp(proc.create_time()).strftime('%Y-%m-%d %H:%M:%S')
                    exe_name = os.path.basename(proc_exe) if proc_exe else "Không xác định"
                    version = (
                        "v4.0.1" if "v4.0.1" in exe_name.lower() or "v4.0.1" in proc_cmdline
                        else "v5.0.3" if "v5.0.3" in exe_name.lower() or "v5.0.3" in proc_cmdline
                        else "Không xác định"
                    )
                    proc_info.append(f"PID: {proc_pid}, Phiên bản: {version}, Tên: {exe_name}, Khởi chạy: {create_time}, Đường dẫn: {proc_exe}")
        ...
        proc_info_str = "\n".join(proc_info)
        msg = (
            f"Phát hiện các phiên bản khác của Flight Ticket Calculator đang chạy:\n\n"
            f"{proc_info_str}\n\n"
            f"Bạn có muốn đóng các phiên bản này để mở phiên bản mới (v5.0.3) không?"
        )

        # Tạo QMessageBox với icon
        msg_box = QMessageBox()
        msg_box.setWindowTitle("Xác nhận đóng phiên bản trùng lặp")
        msg_box.setText(msg)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if still_running:
                ...
                msg_box.setText("Không thể đóng một số phiên bản. Vui lòng đóng thủ công.")
                msg_box.setIcon(QMessageBox.Icon.Critical)
                msg_box.setWindowIcon(QIcon("calculator.ico"))
    ...
    def initUI(self):
        self.setWindowTitle("Flight Ticket Calculator v5.0.3 | Tác giả: Batman")
        ...
        self.main_layout.addWidget(self.round_trip_checkbox)
    ...
    def eventFilter(self, obj, event):
        try:
            ...
            logging.error(f"Lỗi trong eventFilter: {e}")
    ...
    def take_screenshot(self):
        ...
        logging.info("Đã chụp và sao chép ảnh màn hình vào clipboard")
    ...
    def copy_text(self, export_format="Văn bản chi tiết"):
        ...
        try:
            ...
            logging.info(f"Đã sao chép nội dung định dạng '{export_format}' vào clipboard")
            QMessageBox.information(self, "Sao chép nội dung", f"Nội dung định dạng '{export_format}' đã được sao chép vào clipboard.")
        except Exception as e:
            logging.error(f"Lỗi khi sao chép nội dung: {e}")
    ...
    def calculate_total(self):
        try:
            ...
            logging.error("Lỗi nhập liệu trong calculate_total")
    ...
    def clear_fields(self):
        ...
        self.adjust_table_height()
        self.adjust_window_height()
        logging.info("Đã xóa tất cả dữ liệu")
    ...
    def format_price(self):
        ...
        try:
            ...
            logging.error(f"Lỗi định dạng giá: {price_text}")
    ...
    def apply_multiplier(self, trigger_calculate=True):
        ...
        try:
            ...
            logging.error(f"Lỗi áp dụng hệ số giá: {price_text}")
    ...
    def extract_from_clipboard(self):
        try:
            ...
            text = clipboard.text()
            if not text:
                logging.warning("Clipboard không chứa văn bản")
            ...
            self.calculate_total()
            logging.info("Đã nhận dạng và điền dữ liệu từ clipboard")
            QMessageBox.information(self, "Thành công", "Đã nhận dạng và điền dữ liệu từ văn bản!")
            self.adjust_window_height()
        except Exception as e:
            logging.error(f"Lỗi khi nhận dạng clipboard: {e}")
