...
class TicketCalculator(QWidget):
    CURRENT_VERSION = "6.1.1"  # Phiên bản hiện tại
    ...
    def __init__(self):
        ...
        # Tạo mutex và kiểm tra xem đã tồn tại chưa
        self.mutex_name = "Flight_Ticket_Calculator_Mutex"
        self.mutex = win32event.CreateMutex(None, False, self.mutex_name)
        last_error = win32api.GetLastError()

        if last_error == winerror.ERROR_ALREADY_EXISTS:
            # Mutex đã tồn tại, có nghĩa là một phiên bản khác đang chạy
            msg_box = QMessageBox()
            msg_box.setWindowTitle("CẢNH BÁO")
            msg_box.setText("Một phiên bản của Flight Ticket Calculator đã đang chạy.\nBạn có muốn đóng phiên bản hiện tại để mở phiên bản mới không?")
        ...
        # Hiển thị cửa sổ trước khi kiểm tra cập nhật
        self.show()
        self.raise_()
        self.activateWindow()
        ...
        # Thiết lập kiểm tra cập nhật mỗi 2 tiếng
        self.update_timer = QTimer(self)
        self.update_timer.timeout.connect(self.check_for_update)
        self.update_timer.start(7200000)  # 2 tiếng
    ...
    def closeEvent(self, event):
        """Giải phóng mutex khi cửa sổ đóng."""
        try:
            if hasattr(self, 'mutex'):
                win32api.CloseHandle(self.mutex)
        except Exception:
    ...
    def check_for_update(self):
        """Kiểm tra cập nhật và hiển thị thông báo nếu có phiên bản mới."""
        if QApplication.activeModalWidget():  # Bỏ qua nếu có dialog đang mở
            return
        try:
            # Sửa để gọi UpdateManager.check_for_update thay vì Updater.check_for_update
    ...
    def check_running_instances(self):
        ...
        for proc in psutil.process_iter(['pid', 'exe', 'name', 'cmdline']):
            try:
                ...
                # Kiểm tra chính xác hơn: so sánh đường dẫn thực thi
                ...
                running.append(proc)
                proc_info.append(f"+ PID: {proc_pid} | {window_title}")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if still_running:
                still_running_info = "\n".join(f"+ PID: {proc.pid}" for proc in still_running)
                QMessageBox.critical(self, "Lỗi", f"Không thể đóng một số phiên bản:\n{still_running_info}\nVui lòng đóng thủ công.")
                return False
            return True
        return False
    ...
    def show_support_dialog(self):
        ...
        try:
            # Kiểm tra xem có dialog modal nào đang mở không
            if QApplication.activeModalWidget():
                return

            # Kiểm tra trạng thái _instance
            if SupportDialog._instance is not None:
                # Đảm bảo instance còn hợp lệ
                try:
                    ...
                    # Instance có thể đã bị hủy (C++ object deleted)
