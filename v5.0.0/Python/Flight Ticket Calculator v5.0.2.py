...
class TicketCalculator(QWidget):
    def __init__(self):
        ...
        if not self.check_running_instances():  # Kiểm tra các phiên đang chạy
            sys.exit(0)  # Thoát nếu người dùng chọn "No"
        self.initUI()

    def check_running_instances(self):
        """Kiểm tra và đóng các phiên bản khác của ứng dụng nếu người dùng đồng ý. Trả về False nếu người dùng chọn No."""
        current_pid = os.getpid()  # Lấy PID của tiến trình hiện tại
        current_exe_path = os.path.abspath(sys.executable if getattr(sys, 'frozen', False) else __file__).lower()
        running_processes = []
        ...
        for proc in psutil.process_iter(['pid', 'name', 'exe', 'cmdline']):
            try:
                ...
                if 'flight ticket calculator' in proc_name or \
                   'flight ticket calculator' in proc_exe or \
                   'flight ticket calculator' in proc_cmdline:
                    running_processes.append(proc)
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                continue

        # Nếu có các tiến trình khác đang chạy
        if running_processes:
            # Thu thập tên file thực thi để hiển thị trong thông báo
            proc_names = []
            for proc in running_processes:
                try:
                    exe_name = os.path.basename(proc.info['exe']) if proc.info['exe'] else proc.info['name']
                    proc_names.append(exe_name)
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    proc_names.append("Unknown")
            proc_names_str = ", ".join(set(proc_names))  # Loại bỏ trùng lặp
            msg = f"Phát hiện các phiên bản khác của Flight Ticket Calculator đang chạy:\n{proc_names_str}\n\nBạn có muốn đóng các phiên bản này để mở phiên bản mới (v5.0.2) không?"
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Xác nhận đóng phiên cũ")
            msg_box.setText(msg)
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                return False
        return True  # Không có phiên khác, tiếp tục mở ứng dụng

    def initUI(self):
        self.setWindowTitle("Flight Ticket Calculator v5.0.2 | Tác giả: Batman")
