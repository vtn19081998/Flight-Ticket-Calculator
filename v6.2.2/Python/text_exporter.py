...
class TextExporter:
    def __init__(self, airlines_config, parent=None):
    ...
    def copy_text(self, export_format, price_text, discount_text, adult_text, child_text, infant_text,
        ...
        if not all(required_inputs):
            QMessageBox.warning(self.parent, "Thiếu thông tin", "Vui lòng nhập đầy đủ tất cả các trường trước khi Xuất dữ liệu.")
            return
        ...
        try:
            ...
            QMessageBox.information(self.parent, "Sao chép nội dung", f"Nội dung định dạng '{export_format}' đã được sao chép vào clipboard.")
        except Exception as e:
            QMessageBox.critical(self.parent, "Lỗi", f"Có lỗi xảy ra: {str(e)}")